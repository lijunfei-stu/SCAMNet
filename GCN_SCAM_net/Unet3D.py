# todo github:https://github.com/liujiyaoFDU/3D-U-net
...
class UNet3D(Module):
    ...
    def forward(self, x):
        ...
        # 对每个解码器的输出进行上采样，确保尺寸一致
        d4 = torch.cat(
            [F.interpolate(self.deconv_blk4(base), size=x4.shape[2:], mode='trilinear', align_corners=False), x4],
            dim=1)
        d_high4 = self.dec_conv_blk4(d4)

        d3 = torch.cat(
            [F.interpolate(self.deconv_blk3(d_high4), size=x3.shape[2:], mode='trilinear', align_corners=False), x3],
            dim=1)
        d_high3 = self.dec_conv_blk3(d3)

        d2 = torch.cat(
            [F.interpolate(self.deconv_blk2(d_high3), size=x2.shape[2:], mode='trilinear', align_corners=False), x2],
            dim=1)
        d_high2 = self.dec_conv_blk2(d2)

        d1 = torch.cat(
            [F.interpolate(self.deconv_blk1(d_high2), size=x1.shape[2:], mode='trilinear', align_corners=False), x1],
            dim=1)
