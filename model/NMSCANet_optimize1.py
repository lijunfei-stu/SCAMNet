...
"""
基于通道注意力的多尺度全局卷积模块
移植自前列腺分割网络，适配2D立体匹配特征提取，通过分解式1D卷积实现大核感受野，无参数爆炸
核心优势：大感受野捕捉全局上下文，同时保留边缘细节，解决弱纹理区域特征鲁棒性问题
"""
...
class NMSCANet(nn.Module):
    def __init__(self, max_disp=192, in_channels=3, base_channels=32):
        ...
        # 多尺度视差预测头 - 修复：disp_pred_4x输入通道改为base_channels*4（匹配x3的通道数）
        ...
        self.disp_pred_4x = nn.Conv2d(base_channels * 4, 1, kernel_size=3, padding=1)  # 核心修改点
    ...
    def forward(self, left_img, right_img, return_both=True):
        """
        主前向传播函数
        """
        # 预测左视差（左图→右图）
...
if __name__ == "__main__":
    ...
    # 模拟输入：batchsize=1，3通道，1024×1280分辨率（H×W）
    ...
    with torch.no_grad():
        ...
        print(f"推理模式 - 左视差形状: {disp_left.shape}")  # 预期: [1, 1, 1024, 1280]
        print(f"推理模式 - 右视差形状: {disp_right.shape}")  # 预期: [1, 1, 1024, 1280]
    ...
    print(f"训练模式 - 初始右视差形状: {outputs[3].shape}")
